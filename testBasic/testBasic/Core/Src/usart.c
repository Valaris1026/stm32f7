...
volatile uint8_t U1_TX_BUF[WIFI_BUF_MAXLEN] = {0};
volatile uint8_t U1_RX_BUF[WIFI_BUF_MAXLEN] = {0};
volatile uint8_t lwrb2_buf[WIFI_BUF_MAXLEN];
...
int fputc(int ch, FILE *f)
{
	uint8_t tmp;
	tmp = (uint8_t)ch;
	HAL_UART_Transmit(&huart7,&tmp,1,1000);
	return ch;
}

void USART_FSM_Init(void)
{
	lwrb_init((lwrb_t *)&USART1_Token.lwrb_handle, (uint8_t *)lwrb2_buf, WIFI_BUF_MAXLEN);
}
